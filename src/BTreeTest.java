...
public class BTreeTest
{
	...
	/**
	 * Method for binary search trees as guide, write a method to display a B-tree given a reference to its
	 * root.
	 * @param T B-Tree
	...
	{
		if(depth == 0)
		{//if at the correct depth
			return T.item[0];//return the leftmost value
		}
		else
		{//if the correct depth is found return the smallest at that depth
	...
	public static int maxAtDepth(BTreeNode T, int depth)
	{
		...
		{//if the correct depth is found return the largest at that depth
	...
	{
		if(depth == 0)
		{//Print the nodes found at the depth required
			for(int i = 0; i < T.n; i++)
			{
